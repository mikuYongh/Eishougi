      ...
      <div className="flex items-center justify-between flex-shrink-0 bg-black/20 p-4 rounded-2xl border border-white/5 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[12px] text-white/50">{workflow.name || "未命名工作流"}</p>
          </div>
        </div>
      ...
      <div className="flex gap-6 flex-1 min-h-0 overflow-y-auto pb-10">
        ...
        <div className="w-[380px] flex-shrink-0 flex flex-col gap-5">
          <div className="glass-panel p-5 space-y-5">
            <div>
              <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-1.5 block">工作流名称</label>
              <input 
                type="text" value={workflow.name} onChange={e => updateField('name', e.target.value)}
                className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white/90 text-sm outline-none focus:border-yellow-500/50 transition-colors font-bold"
                placeholder="例如：基础文生图"
              />
            </div>
            
            <div>
              <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-1.5 block">用途描述</label>
              <textarea 
                value={workflow.description} onChange={e => updateField('description', e.target.value)}
                className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white/60 text-xs outline-none focus:border-yellow-500/50 transition-colors h-24 resize-none"
                placeholder="工作流的具体用途和使用场景..."
              />
            </div>

            <div className="relative z-50">
              <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-1.5 block">工作流类型</label>
              <GlassDropdown 
                ...
                accentColor="yellow"
              />
              <p className="text-[10px] text-white/40 mt-2">不同的类型将决定它在生成界面的挂载位置。</p>
            </div>
          </div>

          {/* Thumbnail Uploader */}
          <div className="glass-panel p-5">
            <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-3 block">缩略图</label>
            <div className="h-40 w-full rounded-xl border-2 border-dashed border-white/10 hover:border-yellow-400/50 bg-black/30 flex flex-col items-center justify-center relative overflow-hidden group cursor-pointer transition-colors">
              {workflow.thumbnail ? (
                <>
                  <img src={workflow.thumbnail} alt="Thumbnail" className="w-full h-full object-cover opacity-80 group-hover:opacity-60 transition-opacity" />
                ...
                <>
                  <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
                    <UploadCloud size={16} className="text-white/40 group-hover:text-yellow-400 transition-colors" />
                  </div>
                  <span className="text-[11px] font-bold text-white/50">上传缩略图 (可选)</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 glass-panel overflow-hidden">
          <div className="flex items-center justify-between p-3 border-b border-white/5 bg-black/20">
            <h3 className="text-[13px] font-bold text-white/90 flex items-center gap-2 px-2">
              <Code size={16} className="text-yellow-400" /> ComfyUI API JSON (API Format)
            </h3>
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-white/70 text-[11px] font-bold transition-colors cursor-pointer">
              <Cpu size={12} /> 校验并格式化
            </button>
          </div>
