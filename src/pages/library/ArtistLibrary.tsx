...
import type { FavoriteArtist } from "../../stores/favoriteLibraryStore";

export function ArtistLibrary() {
  const [selectedItem, setSelectedItem] = useState<Artist | null>(null);
  const [selectedFavorite, setSelectedFavorite] = useState<FavoriteArtist | null>(null);
  ...
  useEffect(() => {
    ...
    if (artistShowFavorites) {
      favStore.fetchArtists(true);
    }
  ...
  const getImgSrc = (url: string | null | undefined) => {
    ...
    return `https://blobs.animadex.net/ArtistOutputs/thumbs/${encodeURIComponent(url)}`;
  };

  const currentData = artistShowFavorites ? favStore.artists : artists;
  const isLoading = artistShowFavorites ? favStore.isArtistsLoading : isArtistsLoading;
  const loadMore = artistShowFavorites ? favStore.fetchArtists : loadMoreArtists;
  ...
  return (
    <div className="flex flex-col h-full gap-4 relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative group">
            ...
            <input
              ...
              disabled={artistShowFavorites}
              className={`w-full md:w-64 bg-[var(--bg-layer-1)] border border-[var(--glass-border)] rounded-xl py-2 pl-9 pr-4 text-sm text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:border-[var(--accent-1)] transition-all ${artistShowFavorites ? 'opacity-50 cursor-not-allowed' : ''}`}
            />
          </div>
          <button
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        <VirtuosoGrid
          ...
          itemContent={(index, item) => {
            if (artistShowFavorites) {
              const fav = item as FavoriteArtist;
                ...
                >
                  <div className="aspect-square w-full bg-[var(--bg-layer-2)] relative overflow-hidden">
                    {imgSrc ? (
                      <img
                        src={imgSrc}
                        alt={fav.displayName || fav.artistTag}
                        loading="lazy"
            ...
            const imgSrc = getImgSrc(artist.imgUrl);
            return (
              <div 
                className="w-full relative group cursor-pointer rounded-2xl overflow-hidden border border-[var(--glass-border)] bg-[var(--bg-layer-1)] hover:border-[var(--accent-1)]/50 transition-all duration-300 hover:shadow-[0_0_20px_rgba(0,0,0,0.3)]"
      ...
      {selectedItem && !artistShowFavorites && (
        <ItemDetailModal
          item={selectedItem}
      ...
      {selectedFavorite && artistShowFavorites && (
