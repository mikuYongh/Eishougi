...
#[tauri::command]
pub async fn get_favorite_prompts(
    state: State<'_, AppState>,
    prompt_type: String,
) -> Result<Vec<FavoritePrompt>, String> {
    let db = state.db.lock().await;
    let mut stmt = db
        .conn
        .prepare("SELECT * FROM favorite_prompts WHERE type = ?1 ORDER BY created_at DESC")
        .map_err(|e| e.to_string())?;

    let rows = stmt
        .query_map(params![prompt_type], |row| {
            Ok(FavoritePrompt {
                id: row.get("id")?,
...
pub async fn delete_favorite_prompt(state: State<'_, AppState>, id: String) -> Result<(), String> {
    let db = state.db.lock().await;
    db.conn
        .execute("DELETE FROM favorite_prompts WHERE id = ?1", params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
