...
pub async fn get_prompt(state: State<'_, AppState>, id: String) -> Result<Option<Prompt>, String> {
    ...
    let mut stmt = db
        .conn
        .prepare("SELECT * FROM prompts WHERE id = ?1 AND deleted_at IS NULL")
        .map_err(|e| e.to_string())?;
    let prompt = stmt
        .query_row(params![id], |row| {
            Ok(Prompt {
                ...
                prompt_syntax: row
                    .get("prompt_syntax")
                    .unwrap_or_else(|_| "danbooru".to_string()),
                seed: row.get("seed")?,
                width: row.get("width")?,
    ...
    if let Some(mut p) = prompt {
        let mut img_stmt = db
            .conn
            .prepare("SELECT * FROM prompt_images WHERE prompt_id = ?1")
            .map_err(|e| e.to_string())?;
        let img_rows = img_stmt
...
pub async fn update_prompt(
    ...
    println!(
        "[Rust] update_prompt id={}, positive_prompt(first 80)={}, steps={}",
        prompt.id,
    ...
    db.conn
        .execute(
            "DELETE FROM prompt_images WHERE prompt_id = ?1",
            params![prompt.id.clone()],
        )
        .map_err(|e| e.to_string())?;
    ...
    db.conn
        .execute(
            "DELETE FROM prompt_tag_cross WHERE prompt_id = ?1",
            params![prompt.id.clone()],
        )
        .map_err(|e| e.to_string())?;
...
pub async fn list_prompts(
    ...
    let img_query = format!(
        "SELECT * FROM prompt_images WHERE prompt_id IN ({})",
        id_list
    );
